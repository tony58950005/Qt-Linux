...
bool MainWindow::eventFilter(QObject *obj, QEvent *event)
{
    ...
    if(obj == appWidget)
    {
		...
		if(moveOffset>=50){appWidget->move(50,0);}
		//else{appWidget->move(appWidget->pos().x()+moveOffset,0);}
	     }else if(loopCount >0 && moveOffset <0){
                if(moveOffset<=-50){appWidget->move(-50-desktopWidth,0);}
		//else{appWidget->move(appWidget->pos().x()+moveOffset,0);}
...
void MainWindow::appMoveWin(int appIndex)
{
    ...
    if(loopCount <=0 && appBtn[appIndex]->moveOffset >=0 && appBtn[appIndex]->moveOffset>=50){appBtn[appIndex]->moveOffset = 0;mainWinMove();}
    if(loopCount >0 && appBtn[appIndex]->moveOffset <0 && appBtn[appIndex]->moveOffset<=-50){appBtn[appIndex]->moveOffset = 0;mainWinMove();}



    /*if(loopCount <=0 && appBtn[appIndex]->moveOffset >=0){
        if(appBtn[appIndex]->moveOffset>=50){appWidget->move(0,0);}
	//else{appWidget->move(appWidget->pos().x() + appBtn[appIndex]->moveOffset,0);}
    }else if(loopCount >0 && appBtn[appIndex]->moveOffset <0){
        if(appBtn[appIndex]->moveOffset<=-50){appWidget->move(0,0);}//else{appWidget->move(appWidget->pos().x() + appBtn[appIndex]->moveOffset,0);}
    }else{
   ...
   // appWidget->move(appWidget->pos().x() + appBtn[appIndex]->moveOffset,0);
    ...
    appBtn[appIndex]->appFlagTimer->start(20);
}
