    ...
    : QMainWindow(parent)
{
    ...
    this->setStyleSheet("QMainWindow {background-color: rgb(215, 206, 222);}");
    //this->setGeometry(0,0,800,480);
    QFont font;

    topWidget = new QWidget(this);
    topWidget->setGeometry(0,0,this->geometry().width(),40);
    topWidget->setStyleSheet("QWidget{background-color:rgba(45,45,45,100%)}");
    ...
    for (int i = 0; i < 14; i++)
        pbInputArr[i]->setStyleSheet("QPushButton {color: rgb(255, 66, 255);"
                                     "font: "+ strPointSize + "pt \"微软雅黑\";"
                                                              "background-color: rgb(111,111,111);}"
    ...
    labelDataDisplay->setStyleSheet("QLabel { color: black;}");
